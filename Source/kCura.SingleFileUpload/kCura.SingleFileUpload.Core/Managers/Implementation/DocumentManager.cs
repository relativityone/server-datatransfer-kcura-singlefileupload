...
namespace kCura.SingleFileUpload.Core.Managers.Implementation
{
	public class DocumentManager : BaseManager, IDocumentManager
	{
		...
		{
			return ViewerSupportedFileTypes.Any(x => x.TypeExtension.Equals(fileExtension.ToLower()));
		}
		...
		public int GetDocByName(string docName)
		{
			...
			{
				Condition = new Client.TextCondition(DTOs.DocumentFieldNames.TextIdentifier, Relativity.Client.TextConditionEnum.EqualTo, docName),
		...
		public async Task<bool> ValidateFileTypes(string extension, IHelper helper)
		{
			IEnumerable<string> result = await InstanceSettingManager.Instance.GetRestrictedExtensionsAsync(helper);
			List<string> restricted = result.Select(x => x.ToLower()).ToList();
			return !restricted.Contains(extension);
		}
